...
for lang_pair in "${!lang_dict[@]}"; do
    ...
    for score in "${!scores[@]}"; do
        ...
        python main.py --active_config ${lang_dict[${lang_pair}]} --function supervised_train --model mbart --test_size 0 --val_size 0 \
            --epochs 10 --score ${scores[$score]} --selfsup_strategy sample --batch_size 16 \
            --dist_strategy deepspeed_stage_2 --accumulate_grad_batches 1 --dir_name $lang_pair \
            --adapter --max_length 50 --gpu_num 1 --min_epoch 1 \
            --separate_monolingual_dataset --mono_dataset_path bookcorpus --label_train_size 200000 --unlabel_train_size 1
    done
done