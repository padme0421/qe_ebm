...
for lang_pair in "${!lang_dict[@]}"; do
    ...
    for score in "${!scores[@]}"; do
        ...
        python main.py --active_config ${lang_dict[${lang_pair}]} --function semisupervised_train --model mbart --test_size 0 --val_size 0 \
            --epochs 10 --score ${scores[$score]} --selfsup_strategy sample --batch_size 1 \
            --dist_strategy deepspeed_stage_2 --accumulate_grad_batches 16 --dir_name $lang_pair --unsup_wt 0.001 \
            --adapter --max_length 50 --gpu_num 1 --min_epoch 1 --weight_schedule --ebm \
