...
from transformers.generation.utils import ModelOutput, logging, GenerateOutput, BeamSearchOutput, BeamSearchDecoderOnlyOutput, BeamSearchEncoderDecoderOutput
...
class CustomGenerationMixin(GenerationMixin):
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        beam_scores[:, 1:] = -1e9
        beam_scores = beam_scores.clone().view((batch_size * num_beams,))
        ...
        while True:
            ...
            next_token_logits = self.adjust_logits_during_generation(next_token_logits.clone(), cur_len=cur_len)
            #print("[DEBUGGING] CustomGenerationMixin: next_token_logits.shape: ", next_token_logits.shape)
            next_token_scores = nn.functional.log_softmax(
                next_token_logits.clone(), dim=-1
            )  # (batch_size * num_beams, vocab_size)
            #print("[DEBUGGING] CustomGenerationMixin: next_token_scores.shape: ", next_token_scores.shape)
            next_token_scores_processed = logits_processor(input_ids, next_token_scores.clone())
            next_token_scores = next_token_scores_processed.clone() + beam_scores[:, None].expand_as(next_token_scores.clone())
            ...
            next_token_scores = next_token_scores.clone().view(batch_size, num_beams * vocab_size)

            # Sample 2 next tokens for each beam (so we have some spare tokens and match output of beam search)
            next_token_scores, next_tokens = torch.topk(
                next_token_scores.clone(), 2 * num_beams, dim=1, largest=True, sorted=True
            )
